      ...
      <template v-else-if="decision">
        <div class="card text-center py-10 mb-6">
          <template v-if="decision === 'approved'">
            ...
            <p class="text-gray-400 text-xs mt-1">Entrada: <strong class="text-gray-700">{{ formatMoney(negotiation.entrada) }}</strong> via Pix</p>
          </template>
          <template v-else>
        ...
        <div class="space-y-3">
          <template v-if="decision === 'approved'">
            <RouterLink
              :to="`/negociacoes/${negotiation.id}`"
              class="btn-success w-full block text-center text-base py-3"
            >
