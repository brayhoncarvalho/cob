...
// Estado da decisão
const decisao      = ref(null) // 'aprovar' | 'reprovar' | 'contraproposta'
const motivo       = ref('')
...
// Rótulos de feedback por decisão da Mesa
const decisaoInfo = computed(() => {
  switch (decisao.value) {
    case 'aprovar':        return { cor: 'green',  texto: 'Proposta aprovada pela Mesa — aguarda validação da Gerência.' }
    case 'reprovar':       return { cor: 'red',    texto: 'Proposta reprovada — cliente será notificado sem passar pela Gerência.' }
    case 'contraproposta': return { cor: 'amber',  texto: 'Contraproposta enviada para a Gerência validar antes de notificar o cliente.' }
    default: return null
  }
})
          ...
          <div class="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
              ...
              <div>
                ...
                <p class="text-xs text-gray-400">{{ (((neg.desconto) / (neg.totalDivida || contract?.saldoDevedor || 1)) * 100).toFixed(1) }}%</p>
              </div>
            </div>
          </div>
    ...
    <template v-else>
      <div class="grid xl:grid-cols-3 gap-6">
        ...
        <div class="space-y-4">
          ...
          <!-- Botões de decisão -->
          <div class="bg-white rounded-xl border border-gray-200 p-5">
            <h3 class="font-semibold text-gray-900 mb-1">Decisão da Mesa</h3>
            <p class="text-xs text-gray-400 mb-4">Selecione uma ação para este processo</p>
            ...
            <div v-if="decisao === 'aprovar'" class="mb-4">
              ...
              <textarea
                v-model="motivo"
                rows="2"
                placeholder="Observações para a Gerência..."
            ...
            <div v-if="decisao === 'contraproposta'" class="space-y-3 mb-4">
              ...
              <div class="rounded-lg bg-amber-50 border border-amber-500/25 px-3 py-2">
                ...
                <p class="text-xs text-amber-400 mt-0.5">
                  Total: {{ formatMoney(totalContraproposta) }} (entrada + {{ contraParcelas || '?' }}x)
                </p>
              </div>
              ...
              <div>
                ...
                <textarea
                  v-model="motivo"
                  rows="2"
                  placeholder="Motivo da contraproposta para a Gerência..."
                  class="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
            </div>

            <!-- Preview do impacto da decisão -->
            <div v-if="decisaoInfo" class="mb-4 rounded-lg px-3 py-2.5 text-xs"
              :class="{
                ...
                'bg-amber-50 border border-amber-200 text-amber-800': decisaoInfo.cor === 'amber',
              }">
            ...
            >
              ...
              {{ loading ? 'Processando...' : 'Confirmar Decisão da Mesa' }}
            </button>
          </div>
